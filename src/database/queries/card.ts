...
export async function getRemainingCards(deckId: string) {
    return (await db('card_to_deck').where({deck_id: deckId, is_drawn: false}).count({count: '*'}))[0].count;
}
...
export async function insertCardToDeck(deckId: string, suit: Suit, value: CardValue, cardIndex: number, trx?: Knex.Transaction) {
    ...
    const query = db('card_to_deck').insert({ deck_id: deckId, card_code: card.code, card_index: cardIndex });
    if (trx) {
